  ...
  return (
    <nav
      className={`fixed top-4 left-4 right-4 z-50 transition-all duration-300 ${isScrolled
        ? 'glass-crystalline rounded-2xl py-3 px-6'
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          <div className="flex items-center gap-1 bg-secondary/50 rounded-xl p-1 border border-white/5">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`px-5 py-2 rounded-lg transition-all duration-300 font-medium ${location.pathname === link.path
                  ? 'bg-primary text-primary-foreground shadow-[0_0_15px_rgba(57,255,20,0.4)]'
          ...
          <button
            ...
            className="ml-4 w-10 h-10 rounded-xl flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 border border-transparent hover:border-primary/20 transition-all duration-300 icon-bounce"
      ...
      {
        isMobileMenuOpen && (
          <div className="md:hidden mt-4 p-4 rounded-2xl bg-card/90 backdrop-blur-xl border border-primary/20 shadow-[0_0_20px_rgba(57,255,20,0.15)] animate-fade-in-down">
